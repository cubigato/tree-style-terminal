...
class TestShortcutController:
    ...
    def test_terminal_search_action_uses_current_terminal(
        self,
        shortcut_controller,
        session_manager,
    ):
        """Test terminal_search action execution for the current session."""
        mock_session = TerminalSession(pid=123, pty_fd=456, cwd="/test", title="test")
        mock_terminal = Mock()
        session_manager.current_session = mock_session
        ...
        with patch.object(
            ...
            action = shortcut_controller.get_action("terminal_search")
            action.activate(None)

            mock_get_terminal.assert_called_once_with(mock_session)
            mock_terminal.show_search.assert_called_once()
    ...
    def test_terminal_clipboard_actions_without_current_session(
        ...
        with patch.object(session_manager, 'get_terminal_widget') as mock_get_terminal:
            ...
            shortcut_controller.get_action("terminal_search").activate(None)
