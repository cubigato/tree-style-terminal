...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Call-site review confirms each getter (VteTerminal.get_transparency/get_current_theme/force_update_directory_tracking, SessionManager.get_current_theme) has no production or test callers.
- [ ] #2 The dead getters are removed; backing attributes (_transparency, _current_theme, CWD tracking) and their setters/active paths remain unchanged.
- [ ] #3 Any test that only asserted existence of a removed getter is updated or removed.
<!-- AC:END -->
...
<!-- SECTION:NOTES:BEGIN -->
Call-site review (AC #1): get_transparency, get_current_theme (VteTerminal), force_update_directory_tracking, and SessionManager.get_current_theme have zero callers outside their own definitions in src and tests. Backing state and setters are still used.
<!-- SECTION:NOTES:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [ ] #1 Focused terminal-widget and session-manager tests pass.
- [ ] #2 No behavior change beyond removing the unused getters.
<!-- DOD:END -->
